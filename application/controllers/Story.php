...
class Story extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array('story_m','news_m'));
	}
	public function index()
	{
		if (empty($offset)) $offset = 0;
		$this->data['story']=$this->story_m->get_paged_list($this->limit, $offset)->result();
		// generate pagination
		$this->load->library('pagination');
		$config['base_url'] = site_url('story/index/');
		$config['total_rows'] = $this->story_m->count_all();
		...
		$this->template->display('members/story/index',$this->data);
	}
	public function read($seo=NULL)
	{
		$news=$this->story_m->get_story($seo)->row();
	    if (empty($news)) {
	    	show_404();
	    }else{
	    	$news=$this->story_m->get_story($seo)->row();
			...
			//get tweet
			$this->data['tweets']=$connection->get('search/tweets',['q'=>$keyword,'count'=>$tweet_count,'lang'=>'id']);
	    	$this->template->display('members/story/detail',$this->data);
	    }
	}
